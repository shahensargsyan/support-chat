...
//        echo "An error has occurred with user $userId: {$e->getMessage()}\n";
    ...
    function onMessage(ConnectionInterface $conn, $msg){
        ...
        switch ($type) {
            case 'socket':
                $conn->resourceId;
                $this->users[$msg->user_id]['conn'] = $conn;
                $this->users[$msg->user_id]['user'] = [
                    'name' => $msg->name,
                    'user_id' => $msg->user_id,
                ];
                $this->connections[$conn->resourceId]['user_id'] = $msg->user_id;

                if (isset($this->users[1])) {
                    $clients = $this->users;
                    unset($clients[1]);
                    $this->users[1]['conn']->send(json_encode([
                        'type' => 'user',
                        'users' => $clients,
                    ]));
                }
            ...
            case 'chat':
                $this->users[$msg->to]['conn']->send(json_encode([
                    'msg' => $msg->content,
